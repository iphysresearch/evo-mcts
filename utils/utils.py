...
def init_client(cfg):
    ...
    if cfg.get("model", None): # for compatibility
        ...
        if (model.startswith("gpt") or model.startswith("o3") or model.startswith("o1") or model.startswith("claude")):
            from utils.llm_client.openai import OpenAIClient
            client = OpenAIClient(model, temperature, base_url=cfg.get("base_url", None), api_key=cfg.get("api_key", None))
