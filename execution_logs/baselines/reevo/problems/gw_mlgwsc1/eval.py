...
# os.environ["NUMEXPR_MAX_THREADS"] = "96"  # 或者根据您的系统调整

# Define max_workers as a global variable using percentage of available CPUs
CPU_USAGE_PERCENT = 20  # Use 10% of available CPUs
    ...
    try:
        ...
        for res_name in ['PyCBC', 'cWB']:
            try:
                ...
                if hostname == 'Gravitation-wave': # A6000
                ...
                elif hostname == 'login03': # Dongfang 登录节点
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt < max_retries - 1:
                            logging.warning(f"{res_name} file access error (attempt {attempt+1}/{max_retries}): {e}. Waiting {retry_delay} seconds before retry...")
                            import time
                            time.sleep(retry_delay)
                        else:
                            logging.error(f"Failed to access {res_name} file after {max_retries} attempts: {e}")
                            raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        # injection_ids = injection_ids[:4] + injection_ids[5:]
        # injection_ids = injection_ids[:2] # FIXME: remove this
